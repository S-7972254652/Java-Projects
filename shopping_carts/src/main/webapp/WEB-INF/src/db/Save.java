...
public class Save{
	...
	public static final String database = "shopping_cart";
	...
	static{
		try{
			...
			con = DriverManager.getConnection("jdbc:sqlite:/E:/workspace/shopping_cart.db");
		}catch(Exception ex){
			ex.printStackTrace();
		}
	}
	...
	public static int saveState(String state){
		...
		int id = -1;
		try{
			String query = "insert into "+ stateTable + " (state_name) value (?)";
	...
	public static void saveCities(Integer stateId , ArrayList<String> cities){
		try{
			String query = "insert into "+ cityTable + " (state_id , city_name ) value (? , ?)";
			PreparedStatement ps = con.prepareStatement(query);
