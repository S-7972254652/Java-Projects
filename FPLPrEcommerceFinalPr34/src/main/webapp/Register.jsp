...
<body>
	...
	<div
		...
		<form action="register" method="post" class="bg-white"
			onsubmit="return validateForm();" style="width: 500px;">
			...
			<p id="passwordError"
				style="font-size: 13px; font-weight:bolder; color: red; text-align: center; margin-top: 5px;"></p>
			...
			<label for="name" class="ms-3">Password:</label> <input
				...
				name="password" type="password" id="passwordInput"
				placeholder="Enter Your Password.."> <br> <label
				for="name" class="ms-3">Mobile:</label> <input
				style="width: 300px; margin-left: 46px;" name="mobile" type="text"
				id="mobileInput" placeholder="Enter Your Mobile..">
			<p id="mobileError"
				style="font-size: 13px; color: red; text-align: center; margin-top: 5px;"></p>
			...
			<a href="login.jsp" class="text-decoration-none "
				style="margin-left: 300px; margin-bottom: 50px">Existing User ?
				Login</a>
		</form>
	</div>
	<!-- !Login Form End -->
	...
	<script>
		function validateForm() {
			...
			var passwordError = document.getElementById("passwordError"); // Add this line
			...
			// Check if the password contains at least one special character, one number, and one uppercase letter
			if (!/^(?=.*[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?])(?=.*\d)(?=.*[A-Z]).{7,}$/
					.test(passwordValue)) {
				passwordError.textContent = "Password must contain at least one special character, one number, and one uppercase letter.";
				passwordInput.focus();
				return false;
			}
